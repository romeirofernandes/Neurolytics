...
const SimonAnalytics = () => {
  ...
  const downloadPDF = () => {
    try {
      ...
      doc.text('Simon Task Analytics', margin, yPosition);
      yPosition += 20;
      ...
      autoTable(doc, {
        ...
        body: [
          ...
          ['Avg Simon Effect', `${avgSimonEffect}ms`],
      ...
      autoTable(doc, {
        startY: yPosition,
        head: [['ID', 'Age', 'Gender', 'Education', 'City', 'Compatible RT', 'Incompatible RT', 'Simon Effect', 'Accuracy']],
        body: participantData.map(p => [
          p.id, p.age.toString(), p.gender, p.education, p.city,
          `${p.compatibleRT}ms`, `${p.incompatibleRT}ms`, `${p.simonEffect}ms`, `${p.accuracy}%`
        ]),
        theme: 'grid',
        headStyles: { fillColor: [249, 250, 251], textColor: [26, 26, 26], fontStyle: 'bold' },
      ...
      doc.save(`Simon-analytics-${Date.now()}.pdf`);
    } catch (error) {
