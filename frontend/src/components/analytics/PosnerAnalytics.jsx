...
const PosnerAnalytics = () => {
  ...
  const downloadPDF = () => {
    try {
      ...
      doc.text('Posner Cueing Analytics', margin, yPosition);
      yPosition += 20;
      ...
      autoTable(doc, {
        ...
        body: [
          ...
          ['Avg Cueing Effect', `${avgCueingEffect}ms`],
      ...
      autoTable(doc, {
        startY: yPosition,
        head: [['ID', 'Age', 'Gender', 'Education', 'City', 'Valid RT', 'Invalid RT', 'Cueing Effect', 'Accuracy']],
        body: participantData.map(p => [
          p.id, p.age.toString(), p.gender, p.education, p.city,
          `${p.validRT}ms`, `${p.invalidRT}ms`, `${p.cueingEffect}ms`, `${p.accuracy}%`
        ]),
        theme: 'grid',
        headStyles: { fillColor: [249, 250, 251], textColor: [26, 26, 26], fontStyle: 'bold' },
      ...
      doc.save(`Posner-analytics-${Date.now()}.pdf`);
    } catch (error) {
