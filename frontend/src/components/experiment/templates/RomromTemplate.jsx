...
export const NBackTemplate = ({ onComplete }) => {
  ...
  const trialsPerBlock = 20; // Modified to 20 trials
  const totalBlocks = 1; // Modified to 1 block
  const nBack = 2;
  ...
  const handleResponse = (didRespond) => {
    ...
    const result = { block: 1, trial: currentTrial + 1, letter: letter, isMatch: isMatchTrial ? 1 : 0, responded: didRespond ? 1 : 0, correct: isCorrect ? 1 : 0, resultType, rt: didRespond ? rt : 3000 };
  ...
  if (phase === 'blockEnd') {
    ...
    return (
      <Card className="w-full max-w-2xl mx-auto"><CardHeader><CardTitle>Block Complete!</CardTitle><CardDescription>Your Performance</CardDescription></CardHeader><CardContent className="space-y-6"><div className="grid grid-cols-2 gap-4"><div className="bg-green-100 dark:bg-green-900 p-4 rounded text-center"><div className="text-3xl font-bold text-green-600">{blockStats.hits}</div><div className="text-sm">Hits</div></div><div className="bg-blue-100 dark:bg-blue-900 p-4 rounded text-center"><div className="text-3xl font-bold text-blue-600">{blockStats.correctRejections}</div><div className="text-sm">Correct Rejections</div></div><div className="bg-yellow-100 dark:bg-yellow-900 p-4 rounded text-center"><div className="text-3xl font-bold text-yellow-600">{blockStats.misses}</div><div className="text-sm">Misses</div></div><div className="bg-red-100 dark:bg-red-900 p-4 rounded text-center"><div className="text-3xl font-bold text-red-600">{blockStats.falseAlarms}</div><div className="text-sm">False Alarms</div></div></div><div className="text-center p-4 bg-muted rounded"><div className="text-4xl font-bold">{percentage}%</div><div className="text-sm text-muted-foreground">Accuracy</div></div><Button onClick={nextBlock} size="lg" className="w-full">View Results</Button></CardContent></Card>
    );
  }

  return (
    <Card className="w-full max-w-2xl mx-auto"><CardHeader><CardTitle>N-Back Task (2-Back)</CardTitle><CardDescription>Block 1 of 1 - Trial {currentTrial + 1} of {trialsPerBlock}</CardDescription></CardHeader><CardContent className="space-y-6"><div className="text-center text-sm text-muted-foreground">Press <kbd className="px-2 py-1 bg-muted rounded border">M</kbd> if the letter matches 2 back</div><div className="flex flex-col items-center justify-center h-80 relative">{phase === 'letter' && <div className="text-9xl font-bold">{letter}</div>}{phase === 'blank' && feedback && <div className={`text-6xl font-bold ${feedback === '✓' ? 'text-green-600' : 'text-red-600'}`}>{feedback}</div>}{phase === 'blank' && !feedback && <div className="text-6xl font-bold text-muted-foreground opacity-20">•</div>}</div><div className="grid grid-cols-4 gap-2 text-center text-xs"><div><div className="font-bold text-green-600">{blockStats.hits}</div><div className="text-muted-foreground">Hits</div></div><div><div className="font-bold text-blue-600">{blockStats.correctRejections}</div><div className="text-muted-foreground">Correct</div></div><div><div className="font-bold text-yellow-600">{blockStats.misses}</div><div className="text-muted-foreground">Misses</div></div><div><div className="font-bold text-red-600">{blockStats.falseAlarms}</div><div className="text-muted-foreground">False Alarms</div></div></div></CardContent></Card>
  );
};