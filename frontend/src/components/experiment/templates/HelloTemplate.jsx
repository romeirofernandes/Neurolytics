...
export const PosnerTemplate = ({ onComplete }) => {
  ...
  const startTrial = () => {
    ...
    setTimeout(() => { setPhase('cue');
      //Kept the cue duration at 150ms
      setTimeout(() => { setPhase('target'); setStartTime(Date.now());
        // Modified target duration to 2000ms
        setTimeout(() => {
          if(phase === 'target'){ //check if user has responded
            handleResponse(null); //if not, record a null response to avoid freezing.
          }
        }, 2000);
      }, 150);
    }, 750);
